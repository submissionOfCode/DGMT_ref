...
/*
 * Modified version of xmssmt_core_sign_open for DGMT lowest layer of the XMSS tree
 * Created by: 
*/

int xmssmt_core_sign_open_dgmt(const xmss_params *params,
                          unsigned char *m, unsigned long long *mlen,
                          const unsigned char *sm, unsigned long long smlen,
                          const unsigned char *pk)
{
    ...
    const unsigned char *sm1 = (sm + params->sig_bytes + params->n);
    ...
    *mlen = smlen - params->sig_bytes;
    ...
    for (i = 0; i < params->d; i++) {
        ...
        if(i==0){
            ...
            memcpy(ltreepk,leaf,params->n);     //insert ltree root
            memcpy(ltreepk+params->n,sm1,AES_BLOCK_SIZE);    // append encrypted label
            core_hash(params, leaf, ltreepk, params->n+AES_BLOCK_SIZE); //replace the leaf value
        }
