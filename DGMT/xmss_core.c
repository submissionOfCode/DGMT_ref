 ...
 * root node using Merkle's TreeHash algorithm.
 * Expects the layer and tree parts of subtree_addr to be set.
 * Modified version of treehash_dgmt_D for DGMT lowest layer of the XMSS tree
 * Created by: 
 */
static void treehash_dgmt_D(const xmss_params *params,
                     unsigned char *root, unsigned char *auth_path,
                     const unsigned char *sk_seed,
                     const unsigned char *pub_seed,
                     uint32_t leaf_idx, const uint32_t subtree_addr[8],
                     unsigned char *enc_label)
{
    unsigned char stack[(params->tree_height+1)*params->n];
    unsigned char ltreepk[params->n+AES_BLOCK_SIZE];
    unsigned int heights[params->tree_height+1];
    unsigned int offset = 0;
    ...
    for (idx = 0; idx < (uint32_t)(1 << params->tree_height); idx++) {
        ...
        //add the encrypted label with the root of the ltree
        memcpy(ltreepk,stack + offset*params->n,params->n);     //insert ltree root
        memcpy(ltreepk+params->n,enc_label+idx*AES_BLOCK_SIZE,AES_BLOCK_SIZE);    // append encrypted label
        core_hash(params, stack + offset*params->n, ltreepk, params->n+AES_BLOCK_SIZE); //replace the stack value
                                                                                        // with hased value
 ...
 * Format pk: [root || PUB_SEED] omitting algorithm OID.
 */
int xmssmt_core_seed_keypair_dgmt_D(const xmss_params *params,
                             unsigned char *pk, unsigned char *sk,
                             unsigned char *seed, unsigned char *enc_label)
{
    /* We do not need the auth path in key generation, but it simplifies the
       code to have just one treehash routine that computes both root and path
       in one function. */
    ...
    /* Compute root node of the top-most subtree. */
    treehash_dgmt_D(params, pk, auth_path, sk, pk + params->n, 0, top_tree_addr, enc_label);
    memcpy(sk + 2*params->n, pk, params->n);
                     ...
                     const unsigned char *m, unsigned long long mlen)
{
    ...
    *smlen = params->sig_bytes + mlen;
    ...
    //printf("params->full_height %d\n",params->full_height);
    
    //modified one to use the last possible index
    if (idx > ((1ULL << params->full_height) - 1)) {         
...
/*
 * Modified version of xmssmt_core_sign for DGMT lowest layer of the XMSS tree
 * Created by: 
*/
int xmssmt_core_sign_dgmtM(const xmss_params *params,
                     unsigned char *sk,
                     unsigned char *sm, unsigned long long *smlen,
                     unsigned long long mlen, unsigned char *enc_label)
{
    const unsigned char *sk_seed = sk + params->index_bytes;
    const unsigned char *sk_prf = sk + params->index_bytes + params->n;
    const unsigned char *pub_seed = sk + params->index_bytes + 3*params->n;
    ...
    /* Already put the message in the right place, to make it easier to prepend
     * things when computing the hash over the message. */
    memset(sm + params->sig_bytes, 0, mlen);
    *smlen = params->sig_bytes + mlen;
    ...
    for (i = 0; i < params->d; i++) {
        ...
        /* Initially, root = mhash, but on subsequent iterations it is the root
           of the subtree below the currently processed subtree. */
        if(i==0){
        	expand_seed(params, sm, sk_seed, pub_seed, ots_addr);
        }else 
            wots_sign(params, sm, root, sk_seed, pub_seed, ots_addr);
        sm += params->wots_sig_bytes;
        ...
        /* Compute the authentication path for the used WOTS leaf. */
        if(i==0){
            treehash_dgmt_D(params, root, sm, sk_seed, pub_seed, idx_leaf, ots_addr, enc_label);
        }else{
            treehash(params, root, sm, sk_seed, pub_seed, idx_leaf, ots_addr);
        }
        sm += params->tree_height*params->n;
...
/*
Takes the present key and returns the next signing key;
*******Uses xmssmt_core_sign*******************
*******Needs Optimized implementation**********
 * Created by: 
*/
int xmssmt_core_next_key(const xmss_params *params,unsigned char *sk){
	//unsigned char *m = malloc(XMSS_MLEN_NK);
	unsigned char m[32] =  {255,255,255,255,255,255,255,255,
							255,255,255,255,255,255,255,255,
							255,255,255,255,255,255,255,255,
							255,255,255,255,255,255,255,255};
    unsigned char *sm = malloc(params->sig_bytes + XMSS_MLEN_NK);
    unsigned long long smlen;

	
	
	//printf("%d\n",xmssmt_core_sign(params, sk, sm, &smlen, m, XMSS_MLEN_NK));
	xmssmt_core_sign(params, sk, sm, &smlen, m, XMSS_MLEN_NK);
