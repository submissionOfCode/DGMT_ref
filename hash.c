...
/**
 * Modified version of thash_h for IMT tree
 * Created by:
 */

int thash_h_m(const xmss_params *params,
            unsigned char *out, unsigned char *in1, 
            unsigned char *in2, const unsigned char *pub_seed,
            uint32_t addr[8])
{
    unsigned char buf[params->padding_len + 3 * params->n];
    unsigned char bitmask[2 * params->n];
    ...
    for (i = 0; i < params->n; i++) {
        buf[params->padding_len + params->n + i] = in1[i] ^ bitmask[i];
    }
    
    for (i = 0; i < params->n; i++) {
        buf[params->padding_len + 2*params->n + i] = in2[i] ^ bitmask[params->n+i];
    }
